document.addEventListener('DOMContentLoaded', () => {
    ...
    // A linha abaixo não é mais a forma principal de adicionar o evento de abrir.
    ...
    // Substituímos o forEach por um único Event Listener no body.
    // Isso funciona para links que já existem e para os que serão criados dinamicamente.

    document.body.addEventListener('click', (event) => {
        // 1. Verifica se o elemento clicado (ou um de seus pais) é um link com a classe '.abrir-modal'
        const linkAbrir = event.target.closest('.abrir-modal');

        // 2. Se encontrou um link, executa a lógica
        if (linkAbrir) {
            // Previne o comportamento padrão do link (que seria navegar para outra página)
            event.preventDefault(); 
            
            const url = linkAbrir.getAttribute('href');
            if (url) {
                abrirModal(url);
            }
        }
    ...
    // Adiciona um evento para fechar o modal (esta parte já usava uma boa técnica e permanece igual)
